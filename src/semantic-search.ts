...
export class SemanticSearch<T = unknown> {
  ...
  async addDocument(doc: Document<T>): Promise<void> {
    ...
    this.documentTexts.set(doc.text, doc.id);
  ...
  async remove(id: string): Promise<boolean> {
    const text = this.documents.get(id);
    if (text !== undefined && this.documentTexts.get(text) === id) {
      this.documentTexts.delete(text);
    }
  ...
  import(
    ...
    for (const item of data) {
      ...
      this.documentTexts.set(item.text, item.id);
    }
  }
