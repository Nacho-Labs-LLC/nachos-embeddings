...
describe("BedrockProvider", () => {
  ...
  it("redacts sensitive credentials from config", () => {
    const provider = new BedrockProvider({
      credentials: {
        strategy: "explicit",
        accessKeyId: "SENSITIVE_AKID",
        secretAccessKey: "SENSITIVE_SECRET",
        sessionToken: "SENSITIVE_TOKEN",
        profile: "my-profile",
      },
    });
    const config = provider.getConfig();
    expect(config.credentials.strategy).toBe("explicit");
    expect(config.credentials.profile).toBe("my-profile");
    expect(config.credentials).not.toHaveProperty("accessKeyId");
    expect(config.credentials).not.toHaveProperty("secretAccessKey");
  ...
  it("throws for unknown model without custom adapter", () => {
    expect(() => new BedrockProvider({ modelId: "unknown.model" })).toThrow(
