...
describe("TitanV2Adapter", () => {
  ...
  it("formats request with custom dimensions", () => {
    const body = JSON.parse(
      adapter.formatRequest("Hello", { dimensions: 256 }),
    );
  ...
  it("formats request with normalize disabled", () => {
    const body = JSON.parse(
      adapter.formatRequest("Hello", { normalize: false }),
    );
  ...
  it("rejects invalid dimensions", () => {
    expect(() => adapter.validateOptions({ dimensions: 128 })).toThrow(
      "Invalid dimensions",
    );
    expect(() => adapter.validateOptions({ dimensions: 999 })).toThrow(
      "Invalid dimensions",
    );
    expect(() => adapter.validateOptions({ dimensions: "big" })).toThrow(
      "Invalid dimensions",
    );
  });

  it("rejects non-boolean normalize", () => {
    expect(() => adapter.validateOptions({ normalize: "yes" })).toThrow(
      "Invalid normalize",
    );
  });
...
describe("CohereEmbedAdapter", () => {
  ...
  it("formats request with custom input type", () => {
    const body = JSON.parse(
      adapter.formatRequest("query text", { inputType: "search_query" }),
    );
  ...
  it("formats request with custom truncate", () => {
    const body = JSON.parse(
      adapter.formatRequest("text", { truncate: "NONE" }),
    );
  ...
  it("validates valid input types", () => {
    expect(() =>
      adapter.validateOptions({ inputType: "search_document" }),
    ).not.toThrow();
    expect(() =>
      adapter.validateOptions({ inputType: "search_query" }),
    ).not.toThrow();
    expect(() =>
      adapter.validateOptions({ inputType: "classification" }),
    ).not.toThrow();
    expect(() =>
      adapter.validateOptions({ inputType: "clustering" }),
    ).not.toThrow();
  });

  it("rejects invalid input type", () => {
    expect(() => adapter.validateOptions({ inputType: "invalid" })).toThrow(
      "Invalid inputType",
    );
  });
  ...
  it("rejects invalid truncate", () => {
    expect(() => adapter.validateOptions({ truncate: "MIDDLE" })).toThrow(
      "Invalid truncate",
    );
  });
});
...
describe("resolveModelAdapter", () => {
  ...
  it("throws for unknown model", () => {
    expect(() => resolveModelAdapter("unknown.model")).toThrow(
      "Unknown Bedrock model",
    );
    expect(() => resolveModelAdapter("unknown.model")).toThrow("unknown.model");
  });

  it("lists supported models in error message", () => {
    expect(() => resolveModelAdapter("bad")).toThrow(
